...
from .models import BusinessKeyword,InstanceLock

def search(request):
	try:
		s = Search().query('term',bk_name=request.GET.get('q').lower())
		s.execute()
		return render(request, 'search.html', {'page': s})
	except Exception as e:
		print e
		return render(request, 'search.html')

def remove_lock(request):
	user_id = request.GET.get('user_id')
	lock = InstanceLock.objects.get(user = request.user)
	lock.delete()
...
def load_data(request):
	try:
		for k, v in doc.iteritems():
			for n in v:
				BusinessKeyword.objects.get_or_create(bk_id =n['id'],bk_type=k,bk_name=n['name'])
		from django.core.management import call_command
		call_command('update_index')
		return HttpResponse(json.dumps("Success: Please go back and search"), content_type="application/json")
	except Exception as e:
		print e
